...
export const ProjectsSection: React.FC<ProjectsSectionProps> = ({
  ...
  const { t } = useLang();
  const p = t.projects;
  const dataProjects = t.data.projects;

  // Merge static data with translated text
  const localizedProjects: ProjectItem[] = PROJECTS_DATA.map((proj, i) => ({
    ...proj,
    subtitle: dataProjects[i]?.subtitle ?? proj.subtitle,
    description: dataProjects[i]?.description ?? proj.description,
    details: dataProjects[i]?.details ?? proj.details,
  }));
  ...
  const filteredProjects = useMemo(() => {
    ...
    return localizedProjects.filter(
      (proj) =>
        ...
        proj.techStack.some((tag) => tag.toLowerCase().includes(q)) ||
        proj.type.toLowerCase().includes(q)
    );
  }, [searchQuery, localizedProjects]);

  const renderIllustration = (id: string) => {
    switch (id) {
      case 'synthwave': return <SynthwaveIllustration />;
      case 'tornis':   return <TornisIllustration />;
      case 'rekishi':  return <RekishiIllustration />;
      case 'kroma':    return <KromaIllustration />;
      default:         return null;
    }
  };
    ...
    <section id="proyectos" className="section-reveal">
      <div className="projects-header-row animate-on-scroll">
        <div className="projects-header-text">
          ...
          <p className="projects-subtext">{p.subtext}</p>
        </div>
        <div
        ...
        {filteredProjects.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--color-muted)' }}>
            <p style={{ fontSize: '18px', marginBottom: '12px' }}>
              {p.noResults}"{searchQuery}"{p.noResultsEnd}
            </p>
            <button
              ...
              <React.Fragment key={project.id}>
                ...
                {!isLast && (
                  <div className="project-separator" aria-hidden="true">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="var(--color-primary)" opacity="0.3">
