...
// mouth pose presets reverse-engineered from robbowen.digital
const MOUTH_NEUTRAL = { y: 0, d: 'M299.3,461.3c28.3,15.2,81.2,15.2,102.4-0.4' };
const MOUTH_UP = { y: 0, d: 'M300.33,467.31c25.75,8.49,90,6.12,99.4-8.42' };
const MOUTH_DOWN = { y: -2, d: 'M300.7,467.2c26.6,8.8,84.6,6.2,102.7-0.7' };
const MOUTH_DURATION = 450;

function clamp(min: number, max: number, v: number): number {
  return Math.min(Math.max(v, min), max);
}

function parseNums(d: string): number[] {
  const m = d.match(/-?\d+\.?\d*/g);
  return m ? m.map(Number) : [];
}

function buildD(n: number[]): string {
  return 'M' + n[0] + ',' + n[1] + 'c' + n[2] + ',' + n[3] + ',' + n[4] + ',' + n[5] + ',' + n[6] + ',' + n[7];
}

const easeOutQuad = (t: number) => 1 - Math.pow(1 - t, 2);

export const RobbAvatar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const avatarReady = root.querySelector('.avatar--ready') as HTMLElement | null;
    const avatarEl = root.querySelector('.avatar') as HTMLElement | null;
    if (!avatarReady || !avatarEl) return;

    // La animación de intro dura ~2.2s, luego se quita la clase --intro
    const introTimer = setTimeout(() => {
      avatarEl.classList.remove('avatar--intro');
    }, 2200);

    const headYaw = avatarReady.querySelector('.head__yaw') as HTMLElement;
    const headRotations = Array.prototype.slice.call(
      avatarReady.querySelectorAll('.head__rotation')
    ) as HTMLElement[];
    const faceTilts = Array.prototype.slice.call(
      avatarReady.querySelectorAll('.face-tilt')
    ) as HTMLElement[];
    const eyesX = avatarReady.querySelector('.eyes__x') as HTMLElement;
    const eyesY = avatarReady.querySelector('.eyes__y') as HTMLElement;
    const browLeft = avatarReady.querySelector('.eyebrows__left') as HTMLElement;
    const browRight = avatarReady.querySelector('.eyebrows__right') as HTMLElement;
    const earLeft = avatarReady.querySelector('.ears__left') as HTMLElement;
    const earRight = avatarReady.querySelector('.ears__right') as HTMLElement;
    const beard = avatarReady.querySelector('.beard') as HTMLElement;
    const mouthPath = avatarReady.querySelector('.mouth path') as SVGPathElement;
    const mouthSvg = mouthPath.parentElement as unknown as HTMLElement;

    let currentMouth = MOUTH_NEUTRAL;
    let mouthTweenId = 0;

    function setMouth(target: { y: number; d: string }) {
      if (target === currentMouth) return;
      const fromNums = parseNums(currentMouth.d);
      const toNums = parseNums(target.d);
      const fromY = currentMouth.y;
      const toY = target.y;
      currentMouth = target;

      const myId = ++mouthTweenId;
      let start: number | null = null;

      function frame(now: number) {
        if (myId !== mouthTweenId) return;
        if (start === null) start = now;
        const t = clamp(0, 1, (now - start) / MOUTH_DURATION);
        const e = easeOutQuad(t);
        const nums = fromNums.map((v, i) => v + (toNums[i] - v) * e);
        mouthPath.setAttribute('d', buildD(nums));
        mouthSvg.style.transform = 'translateY(' + (fromY + (toY - fromY) * e) + 'px)';
    ...
    let prevQuadrant = 0;
    let quadrantFlips = 0;
    let dizzyTimeout: ReturnType<typeof setTimeout> | undefined;

    function triggerDizzy() {
      if (!avatarEl) return;
      avatarEl.classList.add('dizzy');
      quadrantFlips = 0;
      clearTimeout(dizzyTimeout);
      dizzyTimeout = setTimeout(() => {
        avatarEl.classList.remove('dizzy');
        quadrantFlips = 0;
        prevQuadrant = 0;
      }, 6000);
    }

    function update(targetX: number, targetY: number) {
      if (!avatarEl || avatarEl.classList.contains('dizzy')) return;

      const size = { x: window.innerWidth, y: window.innerHeight };

      const O = (targetX - size.x / 2) / (size.x / 2);
      const Y = (targetY - size.y / 2) / (size.y / 2);
      const C = clamp(-1, 1, (targetX - size.x / 2) / (size.x / 4));
      const L = clamp(-1, 1, (targetY - size.y / 2) / (size.y / 4));

      let quadrant: number;
      if (L < 0 && C < 0) quadrant = 1;
      else if (L < 0 && C >= 0) quadrant = 2;
      else if (L >= 0 && C >= 0) quadrant = 3;
      else quadrant = 4;
      ...
      if (quadrant !== prevQuadrant) {
        ...
        quadrantFlips += 1;
        setTimeout(() => {
          quadrantFlips = Math.max(0, quadrantFlips - 1);
        }, 500);
      }
      if (quadrantFlips >= 5) {
        triggerDizzy();
      ...
      eyesX.style.transform = 'translateX(' + Math.ceil(10 * O) + 'px)';
      const eyeYval = Math.ceil(10 * Y);
      eyesY.style.transform = 'translateY(' + (eyeYval < -3 ? -3 : eyeYval) + 'px)';

      headYaw.style.transform =
        'translateY(' + (L < 0 ? Math.ceil(1 * L) : Math.ceil(3 * L)) + 'px)';

      headRotations.forEach((el) => {
        el.style.transform = 'rotate(' + 3 * C * L + 'deg)';
      });

      faceTilts.forEach((el) => {
        const ty = L < 0 ? Math.ceil(1 * L) : Math.ceil(3 * L);
        el.style.transform =
          'translateY(' + ty + 'px) translateX(' + Math.ceil(1.25 * C) + 'px)';
      });

      earLeft.style.transform =
        'translateX(' + (C < 0 ? Math.ceil(1 * C) : Math.ceil(-1 * C)) + 'px)';
      earRight.style.transform =
        'translateX(' + (C < 0 ? Math.ceil(-1 * C) : Math.ceil(1 * C)) + 'px)';

      browLeft.style.transform =
        'translateY(' + (C < 0 ? -5 + Math.ceil(2 * C) : -5 + Math.ceil(5 * C)) + 'px)';
      browRight.style.transform =
        'translateY(' + (C < 0 ? -3 + Math.ceil(-5 * C) : -3 + Math.ceil(-3 * C)) + 'px)';

      beard.style.transform =
        L < 0 ? 'scaleY(' + (1 + -1 * L * 0.025) + ')' : 'scaleY(' + (1 - 0.025 * L) + ')';

      let mouthTarget;
      if ((C < -0.5 && L < -0.5) || (C > 0.5 && L < -0.5)) mouthTarget = MOUTH_UP;
      else if ((C < -0.5 && L > 0.5) || (C > 0.5 && L > 0.5) || L > 0.5) mouthTarget = MOUTH_DOWN;
      else mouthTarget = MOUTH_NEUTRAL;
      setMouth(mouthTarget);
    }

    // --- drive updates off mousemove, decoupled onto rAF ---
    ...
    const onDeviceOrientation = (e: DeviceOrientationEvent) => {
      if (hasMoved || e.gamma === null || e.beta === null) return;
      const size = { x: window.innerWidth, y: window.innerHeight };
      const M = clamp(-1, 1, e.gamma / 22.5);
      const S = clamp(-1, 1, e.beta / 45);
      const x = size.x / 2 + (size.x / 2) * (-0.66 * M);
      const y = size.y / 2 + (size.y / 2) * (-0.66 * S);
      if (!ticking) {
        ticking = true;
        requestAnimationFrame(() => {
          update(x, y);
  ...
  return (
    <div className="avatar-wrap" ref={wrapRef}>
      {/* hidden defs: clip-paths + stubble pattern used by the face layers */}
      <svg style={{ position: 'absolute', width: 0, height: 0, overflow: 'hidden' }} aria-hidden="true">
        <defs>
          ...
          <pattern id="pattern--stubble" width="83.3" height="97.7" patternTransform="matrix(0.09, 0, 0, -0.09, -7212.72, 6963.89)" patternUnits="userSpaceOnUse" viewBox="0 0 83.3 97.7">
            <rect className="svg-blank" width="83.3" height="97.7"></rect>
            <rect className="svg-blank" width="83.3" height="97.7"></rect>
            <rect className="svg-beard-fill svg-fill" fill="#addded" x="41.6" y="-24.5" width="83.3" height="48.9"></rect>
            <circle className="svg-bg" fill="#f4f9fc" cx="66.3" cy="-0.3" r="17"></circle>
            <rect className="svg-beard-fill svg-fill" fill="#addded" x="-41.7" y="-24.5" width="83.3" height="48.9"></rect>
            <rect className="svg-beard-fill svg-fill" fill="#addded" y="24.3" width="83.3" height="48.9"></rect>
            <circle className="svg-bg" fill="#f4f9fc" cx="24.6" cy="48.6" r="17"></circle>
            <rect className="svg-beard-fill svg-fill" fill="#addded" x="41.6" y="73.2" width="83.3" height="48.9"></rect>
            <circle className="svg-bg" fill="#f4f9fc" cx="66.3" cy="97.4" r="17"></circle>
            <rect className="svg-beard-fill svg-fill" fill="#addded" x="-41.7" y="73.2" width="83.3" height="48.9"></rect>
          </pattern>
        </defs>
      </svg>
            ...
            <div className="head__y">
              <div className="head__yaw">
                <div className="head__rotation">
                  ...
                  <div className="beard">
                    <svg viewBox="0 0 702 726">
                      <path className="beard__tache svg-pattern" d="M365.9,425.2a23.71,23.71,0,0,1-29.83,0c-25.53,1.6-44.4,18.82-44.4,18.82A187.24,187.24,0,0,0,410,444S391.3,426.82,365.9,425.2Z"></path>
                  ...
                  <div className="eyebrows face-tilt">
                    <div className="eyebrow eyebrows__left">
                      <svg viewBox="0 0 702 726">
                        ...
                        <path className="brow svg-fill-stroke" d="M310.54,277.83l-1,5.81a4.8,4.8,0,0,1-7,3.29c-12.68-6.77-45.49-18.19-78.11,15.2,1.65-17.62,28.26-47.23,83-29.23A4.37,4.37,0,0,1,310.54,277.83Z"></path>
                      </svg>
                    </div>
                  ...
                  <div className="face-tilt">
                    <div className="mouth">
                      <svg viewBox="0 0 702 726">
                        <path className="svg-stroke" d="M299.3,461.3c28.3,15.2,81.2,15.2,102.4-0.4"></path>
                      </svg>
                    </div>
                  </div>
                  ...
                  <div className="hair face-tilt">
                    <svg viewBox="0 0 702 726">
                      <path className="svg-stroke" d="M409.55,174.85c25.85,8.42,53,22.56,70.66,38.23-.87-2.61-1.84-5.12-2.71-7.54.68.67,1.26,1.35,1.93,2A170,170,0,0,0,475,188.4a214.86,214.86,0,0,1,28.17,12.68c-14.52-40.36-39.2-63.49-68-75.1a174.68,174.68,0,0,1,42,7.45C450.79,87,410.62,77.1,371.23,83.78c4.55-3.1,9.77-6.39,15.48-9.87-31.84-2.71-55.84,5.9-73.65,18.19,5-5.51,11-14.32,18-20.81-90.2,13.84-110.14,92-114.3,113.24-4.77,19.33,15.48,30.1,15.48,30.1s39.68-22.16,95.34-33.68c25.64,3.87,52.45,10,68.71,24.49a168.73,168.73,0,0,0-13.35-31.17C391.65,174.18,400.55,174.27,409.55,174.85Z"></path>
